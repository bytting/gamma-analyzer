...
namespace crash
{
    public partial class FormROITable : Form
    {
        ...
        private void FormROITable_Load(object sender, EventArgs e)
        {
            ...
            for(int i=0; i<settings.ROIList.Count; i++)
            {
                ((TextBox)tableLayoutROI.GetControlFromPosition(0, i + 1)).Text = settings.ROIList[i].Name;
                ((TextBox)tableLayoutROI.GetControlFromPosition(1, i + 1)).Text = settings.ROIList[i].StartChannel.ToString();
                ((TextBox)tableLayoutROI.GetControlFromPosition(2, i + 1)).Text = settings.ROIList[i].EndChannel.ToString();
                ((CheckBox)tableLayoutROI.GetControlFromPosition(3, i + 1)).Checked = settings.ROIList[i].Active;
            }            
        }        

        private void btnOk_Click(object sender, EventArgs e)
        {
            settings.ROIList.Clear();
            if(!String.IsNullOrEmpty(tbName1.Text) && !String.IsNullOrEmpty(tbStart1.Text) && !String.IsNullOrEmpty(tbEnd1.Text))
                settings.ROIList.Add(new ROIData(tbName1.Text, Convert.ToInt32(tbStart1.Text), Convert.ToInt32(tbEnd1.Text), cbActive1.Checked, KnownColor.Red.ToString()));
            if(!String.IsNullOrEmpty(tbName2.Text) && !String.IsNullOrEmpty(tbStart2.Text) && !String.IsNullOrEmpty(tbEnd2.Text))
                settings.ROIList.Add(new ROIData(tbName2.Text, Convert.ToInt32(tbStart2.Text), Convert.ToInt32(tbEnd2.Text), cbActive2.Checked, KnownColor.Blue.ToString()));
